...
class ARKServerManagerApp(ctk.CTk):
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            try:
                ...
                _pil = Image.open(_resource_path(os.path.join("ig", "ark_manager.png"))).resize((32, 32))
                self._app_icon = ImageTk.PhotoImage(_pil)
                self.iconphoto(True, self._app_icon)  # type: ignore[reportArgumentType]
            except Exception:
                pass
    ...
    def _download_mod(self, server_id: str, mod_id: str) -> None:
        ...
        self.mod_manager.download_mods(
            ...
            on_done=lambda ok: self.after(0, lambda: self._refresh_mods_list(server_id)),  # type: ignore[reportArgumentType]
        )
    ...
    def _check_updates_on_start(self) -> None:
        ...
        self.update_checker.check_async(
            url, on_result=lambda info: self.after(0, lambda: self._on_update_result(info)))  # type: ignore[reportArgumentType]
    ...
    def _check_updates_manual(self) -> None:
        ...
        self.update_checker.check_async(
            ...
            on_result=lambda info: self.after(
                0, lambda: self._on_update_result(info, manual=True)))  # type: ignore[reportArgumentType]
    ...
    def _start_download_update(self) -> None:
        ...
        self.update_checker.download_and_install(
            info,
            on_done=lambda ok, msg: self.after(0, lambda: self._on_download_done(ok, msg)),  # type: ignore[reportArgumentType]
        )
