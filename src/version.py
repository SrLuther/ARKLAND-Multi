...
CHANGELOG: list[dict] = [
    ...
    },
    {
        ...
        "date": "2026-08-04",
        "changes": [
            "Fix (Web Store / DB): circuit breaker half-open — após cooldown "
            "admite 1 probe; sucesso em query normal fecha o circuit (já não "
            "depende só do ping de diagnostics); falha do probe reabre.",
            "Fix (Web Store / Auditoria): resposta 503 db_circuit_open com "
            "user_message, cooldown_remaining_s e retry_after_s; UI de "
            "Auditoria/doações mostra orientação em vez do código cru.",
            "Melhoria (Web Store / DB): defaults connect/read timeout 5s "
            "(antes 3s) para MySQL remoto — menos falsos positivos no circuit.",
            "Fix (CustomShop 1.10.39): retries de /delivered e /release sob "
            "db_circuit_open — previne ENTREGANDO órfão; claim continua a "
            "reabrir stale na Web Store.",
            "Fix (Web Store / Event Hunt): @api_key_required() nas rotas "
            "plugin Mode B (sem parênteses sobrescrevia endpoint Flask).",
    ...
    },
    {
        ...
        "date": "2026-07-30",
        "changes": [
            "Feat (Web Store / Auditoria Dinos): código público padronizado "
            "({letra}{variante}{género}{seq}, ex. R12347) — mapeamento estável "
            "por família/variante, persistido em catalog_dino_generations e "
            "exibido na API/UI; canonical ARK mantido para correlação.",
            "Feat (CustomShop 1.10.36): dino_records incluem gender do spawn "
            "para o código público da auditoria.",
            "Feat (CustomShop 1.10.37 + Web Store): /checar na cryopod "
            "consulta POST /api/plugin/catalog-dino/lookup e mostra public_code "
            "(ex. R12347) no chat.",
            "Fix (Web Store / Kits): remoção dos *_pack10 também em "
            "WEBSTORE/config.json do ambiente de teste — o boot reintroduzia "
            "os kits a partir da cópia «mais rica».",
        ],
    },
    {
        "version": "1.10.102",
        "date": "2026-07-30",
        "changes": [
            "Feat (Web Store / Catálogo): auditoria pública de dinos L1/L200 "
            "gerados via catálogo — tabela catalog_dino_generations, "
            "GET /api/public/catalog-dinos e aba «Auditoria Dinos».",
            "Feat (CustomShop 1.10.35): captura DinoID1/DinoID2 no spawn e envia "
            "dino_records em /api/pending/delivered (1 registro por dino spawnado).",
            "Remoção (Catálogo): 191 kits *_pack10 L1 de breeding (188 fêmea + "
            "tekstrider/xenomorph/xenomorphgen2 sem Gender) — unitários L1 "
            "permanecem; kit_alfa/beta/gamma e starter intactos.",
            "Feat (SeasonLand): recompensas premium 21/23 passam de "
            "moschops_pack10/parasaur_pack10 para dinos unitários moschops/parasaur L1.",
        ],
    },
    {
        "version": "1.10.101",
    ...
    },
    {
        ...
        "date": "2026-07-28",
        "changes": [
            "Fix (Web Store / Membros por mapa): texto dos chips MAPAS ASSOCIADOS "
            "com contraste claro no tema escuro.",
        ],
    },
    {
        "version": "1.10.99",
    ...
    },
    {
        ...
        "date": "2026-07-27",
        "changes": [
            "Fix (Web Store / Home): mapas com label curta (CRYSTAL, GEN2, VOLCANO, …) "
            "voltam a mostrar a descrição — match por aliases de slug + fallback ao texto padrão.",
        ],
    },
    {
        "version": "1.10.96",
    ...
    },
    {
        ...
        "date": "2026-07-26",
        "changes": [
            "Feat (TEK / Crashes): análise automática com a IA integrada ao detectar "
            "crash — usa NVIDIA/OpenAI (cloud_credentials) + cauda do ShooterGame.log; "
            "sem API key, enriquece com heurística local em vez de «Causa não "
            "identificada» genérica; Fatal error! passa a incluir preamble do log.",
        ],
    },
    {
        "version": "1.10.92",
    ...
    },
    {
        ...
        "date": "2026-07-21",
        "changes": [
            "Feat (Web Store / ARKBANK): extrato da tesouraria com filtro por "
            "Steam ID, intervalo de datas e coluna Jogador (nick Steam).",
            "Test (ARKBANK): list_transactions filtra steam_id/data e enriquece "
            "player_name a partir de store_users.",
        ],
    },
    {
        "version": "1.10.88",
        "date": "2026-07-21",
        "changes": [
            "Feat (TEK / Auto-start): servidores com «Iniciar ao abrir o app» "
            "sobem automaticamente após reconnect no boot (checkbox em "
            "Gerenciamento Automático por servidor).",
            "Melhoria (TEK / Windows): «Iniciar com o Windows» sincroniza o "
            "registro Run ao abrir o app, não só ao salvar Configurações Globais.",
        ],
    },
    {
        "version": "1.10.87",
        ...
        "changes": [
            ...
            "(antes podia ficar em 10 após permanentes ou mix curto); "
            "ensure faz top-up sem reiniciar o timer; header admin mostra "
            "contagem real; galeria pública usa o mesmo ensure.",
            "Test (Web Store / Vitrine): fallback 9+3+3 incompleto com "
            "pool ≥15 → len==15; top-up após permanentes; pool <15.",
        ],
    },
    {
        "version": "1.10.86",
        "date": "2026-07-20",
        "changes": [
            "Feat (Web Store / Encomenda): vitrine em grade 5×3 (15 slots "
            "rotativos; mix 9 grande + 3 médio + 3 pequeno) + até 5 permanentes.",
        ],
    },
    {
        "version": "1.10.85",
    ...
    },
    {
        ...
        "changes": [
            ...
            "/Game/Mods/{ActiveMods[0]}/MapName — o 1.10.72 fazia isso "
            "para Amissa e quebrava Crystal/Gen2 (ActiveMods[0]=S+/stack); "
            "CLI volta a CrystalIsles?listen… e MaxPlayers da config.",
            "Fix (TEK / Painel): «Todas as opções» deixa de sobrescrever "
            "StringVars da Administração (mapa, slots, portas…) — edits "
            "não salvos já não revertiam ao valor antigo do perfil.",
            "Melhoria (TEK / UI): badge ●/↺ no campo Mapa (vanilla ou mod).",
            "Test (TEK / Launch): regressão CrystalIsles/Gen2 + ActiveMods; "
            "expansão Amissa só quando o .mod declara o mapa.",
        ],
    },
    {
        "version": "1.10.83",
    ...
    },
    {
        ...
        "changes": [
            ...
            "CustomShop/Dino Lab); empacotado no build/PyInstaller.",
            "Perf (Web Store / Entregas): cache curto de fila vazia por "
            "steam_id (TTL 8s, ARKSHOP_PENDING_EMPTY_CACHE_TTL_SEC) em "
            "GET/claim pending — reduz DB quando não há pedidos; invalida "
            "ao criar/reabrir Order PENDENTE.",
            "Perf (Web Store / Entregas): GET /api/pending load_only + "
            "snapshot TTL 3s (ARKSHOP_PENDING_GET_CACHE_TTL_SEC); header "
            "X-Pending-Cache.",
            "Feat (Web Store / Entregas): POST /api/pending/batch — status "
            "em lote por steam_ids (opt-in; CustomShop C++ ainda claim-only).",
            "Perf (Web Store / Entregas): recover_stale_entregando fast-path "
            "SELECT 1 LIMIT 1; scheduler RCON documenta uso de "
    ...
    },
    {
        ...
        "date": "2026-07-20",
        "changes": [
            "Fix CRÍTICO (Web Store / Cache): bootstrap localStorage/IndexedDB "
            "isolado por steam_id — evita vazamento me/kit_limits/entitlements "
            "entre contas; logout limpa cache; purge chaves legacy v1.",
            "Fix (Web Store / PIX): polling PIX e carrosséis pausam com "
            "document.hidden — reduz XHR em background.",
            "Fix (Web Store / Tribe): remove time.sleep(1) pós-RCON em "
            "POST /api/tribe/sync — liberta worker Waitress.",
            "Fix (Web Store / Audit): pending_polled só persiste quando há "
            "itens na fila — reduz writes MySQL (sem alterar entrega).",
            "Fix CRÍTICO (Web Store / Mercado Pago): validação HMAC "
            "x-signature no webhook quando MP_WEBHOOK_SECRET configurado.",
            "Fix (Web Store / Boot): warn/fail se ARKSHOP_API_KEY ausente "
            "em produção (ARKSHOP_REQUIRE_API_KEY / ARKSHOP_API_KEY_FAIL_BOOT).",
            "Perf (Web Store / DB): índice hot-path "
            "ix_orders_status_created (status, created_at).",
            "Other: .gitignore servers.json e configs CustomShop locais; "
            "templates servers.json.example e config.json.example.",
            "Other: CrossChat template PollIntervalSeconds 2→5s em docs/config.json.",
            "Perf (Web Store / API): COUNT(*) OVER() em /api/player/history e "
            "/api/player/donations — uma query por endpoint.",
            "Feat (Web Store / Minha Área): GET /api/player/myarea agregado "
            "(summary + 1ª página histórico/doações); frontend usa bundle inicial.",
            "Perf (Web Store / PIX): poll frontend backoff inicial 8s "
            "(alinhado ao throttle MP backend).",
            "Perf (Web Store / Cache): ETag + If-None-Match em /api/catalog "
            "e /api/public/home → 304.",
            "Perf (Web Store / Mercado): expire_stale_claims removido de "
            "listagens (só scheduler + entrega).",
            "Perf (Web Store / Entitlements): cache in-memory TTL 30s por "
            "jogador; invalidação em grant/revoke.",
            "Perf (Web Store / Âmbarômetro): TTL in-memory 60→120s "
            "(ARKSHOP_AMBER_STATS_CACHE_TTL_SEC).",
            "Perf (Web Store / Catálogo): paginação DOM itens 48 cards/página.",
            "Feat (Web Store / Admin): sync-all-permissions e catalog-feed "
            "202 + worker pool arkshop-admin-heavy.",
            "Feat (Web Store): idempotency keys persistentes SQLite "
            "(idempotency_keys.sqlite).",
            "Feat (Web Store / Observabilidade): GET /api/admin/metrics — "
            "pool, cache hit ratio, slow queries.",
            "Other: script k6 smoke tools/k6/webstore_smoke.js + docs/load_test_k6.md.",
            "Fix (Web Store / Saldo): débito players.points atómico "
            "WHERE points >= price (defer optimistic lock version).",
            "Test (Web Store): test_audit_optimizations, test_mp_webhook_signature; "
            "test_pwa service-worker v5.",
        ],
    },
    {
        "version": "1.10.79",
        ...
        "changes": [
            ...
            "Fix P0 (Web Store / DB): pool MySQL alinhado a Waitress 32 "
            "threads — ARKSHOP_DB_POOL_SIZE 15→32, MAX_OVERFLOW 30→32, "
            "recycle 600→280 (abaixo do wait_timeout); teardown "
            "rollback+remove; force release com rollback antes de close.",
            "Fix P0 (Web Store saturação): home/catálogo — build NUNCA no "
            "lock (antes empilhava todas as threads Waitress → fila 70+, "
            "home 55s); SWR + single-flight; /api/* 500 sempre JSON; "
            "amber-stats force-release; Waitress backlog 2048. Verificar: "
            "X-Home-Cache HIT/STALE, duration_ms≪fila, task queue depth↓.",
            "Perf (Web Store / Waitress): default threads 8→32, channel_timeout "
            "180s, connection_limit 500 (ARKSHOP_HTTP_THREADS / "
            "ARKSHOP_CHANNEL_TIMEOUT / ARKSHOP_CONNECTION_LIMIT; cap pool "
            "só com ARKSHOP_HTTP_THREADS_CAP_TO_POOL=1).",
            ...
            "Perf P1 (Web Store / Home): GET /api/public/home — cache "
            "in-memory TTL 45s + build numa passagem (1× catálogo, 1× "
            "servers sem decrypt RCON, sessão DB mural com force release); "
            "invalida em catálogo/mural; log duration_ms + X-Home-Cache; "
            "alvo <2–3s típico (hit ~ms) vs 50s+ sob fila Waitress.",
            "Test (Web Store / Home): cache HIT/MISS, invalidate catálogo/"
            "admin card, sem decrypt RCON na home.",
            "Fix (Web Store / Equipes): freeze pós-confirmação do sorteio "
            "bloqueia só kick (não joins); novo membro recebe +N números.",
            "Fix P0 (Web Store / Catálogo): resposta HTML/DOCTYPE (Waitress/"
            "proxy saturado) já não rebenta com «Unexpected token <»; "
            "mensagem clara em PT + retry 1× em fetchJson, /api/catalog e "
            "aba Disponível.",
            "Perf P1 (Web Store / Catalog+Bootstrap): GET /api/catalog e "
            "parte partilhada de /api/store/bootstrap — cache in-memory TTL "
            "45s (single-flight); invalida em catálogo/settings; "
            "X-Catalog-Cache; bootstrap pré-lê shop_config antes da sessão "
            "DB + force release; alvo hit ~ms vs enrich O(n) sob fila.",
            "Test (Web Store / Catalog): cache HIT/MISS, bootstrap reutiliza "
            "cache, invalidate via shop_config.",
            "Perf P1 (Web Store / Scheduler): skip-if-busy + pausa entre jobs "
            "+ batches menores (auto_cancel/claims/outbox); claims commit por "
            "linha e outbox commit_every — evita long_transaction / pool "
            "starvation sob carga MySQL.",
        ],
    },
    {
        "version": "1.10.78",
    ...
    },
    {
        ...
        "changes": [
            ...
            "(teams_lottery_numbers_per_member); forfeit_on_depart / legacy_keep; "
            "UI Admin Configurações.",
            "Test (Web Store / Equipes): freeze bloqueia kick/join; leave forfeit; "
            "forfeit_on_depart no kick; legacy_keep Q9.",
            "Feat (Web Store / Admin): Gerenciar Jogadores — entrega do catálogo completo "
            "(Itens, Dinos, Dinos 200, Kits) via seletor + busca + qty/motivo → pedido "
            "PENDENTE (mesmo fluxo do kit); POST /api/admin/players/<id>/deliver; audit "
            "admin_player_item_deliver / admin_player_dino_deliver / admin_player_kit_deliver; "
            "licenças continuam no bloco Conceder/Revogar.",
            "Test (Web Store / Admin): entrega admin item + dino L1 + dino200 + kit; rejeita "
            "licença via /deliver; deliver_catalog no detalhe do jogador.",
            "Feat (Web Store / SeasonLand): área admin dedicada «SeasonLand — Ops» "
            "(nav) — claims/eventos season_pass_*, fila sp:/skip-kit|sp:, filtros "
            "steam/status/track/level/data, badge SeasonLand, REENVIAR + resend Â/item; "
            "GET /api/admin/season-pass/orders; Pedidos oculta SeasonLand por omissão "
            "(toggle Incluir SeasonLand + link Ops).",
    ...
    },
    {
        ...
        "date": "2026-07-19",
        "changes": [
            "Feat (Web Store / SeasonLand): recovery stale ENTREGANDO shop/kit (sp:) — "
            "config shop_stale_entregando_minutes; auto → PENDENTE + last_error + audit "
            "shop_stale_entregando_recovered (espelha custom_dino).",
            "Feat (Web Store / CustomShop 1.10.27): release com fail_reason; após N falhas "
            "idênticas (shop_identical_fail_max, default 5) → ERRO + audit staff "
            "(não reabre PENDENTE forever).",
            "Feat (Web Store / SeasonLand): delivery_state real (queued/delivering/delivered/"
            "failed/partial) nas tracks e Minha Área/Disponível; toast claim enfileirado "
            "(warning); /shop = forçar entrega agora; in_game_delivered deprecated.",
            "Feat (Web Store / SeasonLand): estado parcial Â OK + item pendente; admin "
            "resend missing_only (só catálogo em falta/ERRO); GET "
            "/api/admin/season-pass/sku-health + delivery-metrics (ops painel).",
            "Test (Web Store / SeasonLand): 10 testes Phase A/B — stale shop/kit, ERRO após N, "
            "flags queued, available status, partial, sku sync, metrics, reclaim missing.",
            "Fix (Equipes / armazém): ícones element_ore ↔ element_dust (Minério / Pó) estavam invertidos.",
            "Feat (Web Store / Equipes): armazém e «Aplicar ao marco» em grelha com ícone — qty "
            "(ou qty/cap se API tiver warehouse_caps) e aplicado/necessário + APLICAR.",
        ],
    },
    {
        "version": "1.10.75",
