...
def build_db_manager_panel(app: "ARKTEKApp", parent: ctk.CTkFrame) -> None:
    ...
    _btn_disconnect = ctk.CTkButton(conn_bar, text="Desconectar", width=100, height=30,
                                    ...
                                    font=ctk.CTkFont(family="Segoe UI", size=11),
                                    state="disabled")
    _btn_disconnect.grid(row=0, column=col, padx=(0, 14), pady=8)

    # ── Split: árvore esquerda + painel direito ────────────────────────────
    split = ctk.CTkFrame(body, fg_color=bg, corner_radius=0)
    split.grid(row=2, column=0, sticky="nsew", padx=12, pady=(0, 10))
    ...
    def _do_setup_db() -> None:
        sql_file = _ROOT / "setup_db.sql"
        if not sql_file.exists():
            _show_msg("Arquivo não encontrado",
                      f"setup_db.sql não encontrado em:\n{sql_file}")
            return
        _show_sql_file_dialog(sql_file, title="Setup — Banco de dados limpo")
    ...
    def _show_sql_file_dialog(sql_file: Path, title: str = "Executar SQL") -> None:
        """Mostra o conteúdo do .sql e oferece botão para executar."""
        if not state.is_connected():
        ...
        txt.insert("end", sql_file.read_text(encoding="utf-8"))
