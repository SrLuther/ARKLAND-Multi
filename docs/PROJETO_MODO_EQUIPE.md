...
| R4 | **Q9:** Após confirmação, política `teams_lottery_post_confirm_policy` (default **`freeze`**): bloqueia kick até o draw; joins permitidos (+N números); saída perde N números (`teams_lottery_numbers_per_member`, default 2). Alternativas: `forfeit_on_depart` (kick/saída devolvem N ao pool) e `legacy_keep` (números ficam; kick permitido — Q9 antigo) |
...
| Q9 | Números sorteio após kick | Ficam / Queimam / Congelar | **`freeze` (padrão)** — bloqueia kick até o draw; joins OK (+N); saída perde N nº; `forfeit_on_depart` / `legacy_keep` via `teams_lottery_post_confirm_policy` |
