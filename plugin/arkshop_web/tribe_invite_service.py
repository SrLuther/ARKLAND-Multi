...
def get_or_stub_construction_limits(
    db: Session,
    ...
    if cluster_group_id:
        row = db.execute(
            text("""
                SELECT id, cluster_group_id, principal_max, fob_max, notes, updated_at
                FROM tribe_construction_limits
                WHERE cluster_group_id = :gid
                ORDER BY id DESC LIMIT 1
            """),
            {"gid": cluster_group_id},
    ...
    if not row:
        row = db.execute(
            text("""
                SELECT id, cluster_group_id, principal_max, fob_max, notes, updated_at
                FROM tribe_construction_limits
                WHERE cluster_group_id IS NULL
                ORDER BY id DESC LIMIT 1
            """),
        ).fetchone()
    # Jul/2026: limites de construção NÃO são enforced (dois membros do grupo
    # podem ter Principal em mapas diferentes). Stub só informa política.
    deferred = (
        "Limites de construção adiados — sem enforcement. "
        "Motivo: dois membros do mesmo grupo podem ter cada um uma Principal em mapas distintos."
    )
    breeding = (
        "Breeding oficial da tribo apenas no mapa Principal "
        "(regulamento; sem bloqueio técnico)."
    )
    if not row:
        return {
            "principal_max": 0,
            ...
            "enforced": False,
            "deferred": True,
            "breeding_policy": breeding,
        }
    return {
        "id": row[0],
        "cluster_group_id": row[1],
        "principal_max": int(row[2] or 0),
        "fob_max": int(row[3] or 0),
        "notes": row[4] or deferred,
...
def save_construction_limits(
    ...
    *,
    ...
    """Persiste notas opcionais — NÃO ativa enforcement (decisão Jul/2026)."""
    now = _naive(_utcnow())
    # Zera máximos: valores na UI não implicam regra dura.
    principal_max, fob_max = 0, 0
    ...
    if cluster_group_id:
        existing = db.execute(
            text("SELECT id FROM tribe_construction_limits WHERE cluster_group_id = :gid"),
            {"gid": cluster_group_id},
        ).fetchone()
    else:
        existing = db.execute(
            text("SELECT id FROM tribe_construction_limits WHERE cluster_group_id IS NULL LIMIT 1"),
        ).fetchone()
    if existing:
        db.execute(
            text("""
                UPDATE tribe_construction_limits
                SET principal_max = :p, fob_max = :f, notes = :n,
                    updated_at = :now, updated_by_steam_id = :by
                WHERE id = :id
            """),
            {
                "p": int(principal_max), "f": int(fob_max), "n": notes,
                "now": now, "by": admin_steam_id, "id": existing[0],
            },
        )
    else:
        db.execute(
            text("""
                INSERT INTO tribe_construction_limits
                  (cluster_group_id, principal_max, fob_max, notes, updated_at, updated_by_steam_id)
                VALUES (:gid, :p, :f, :n, :now, :by)
            """),
            {
                "gid": cluster_group_id, "p": int(principal_max),
                "f": int(fob_max), "n": notes, "now": now, "by": admin_steam_id,
            },
        )
    db.commit()
    return get_or_stub_construction_limits(db, cluster_group_id)
