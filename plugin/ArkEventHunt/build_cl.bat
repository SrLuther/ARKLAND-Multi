...
echo === Compiling ArkEventHunt (v%PLUGIN_VER%) ===
"%CL_EXE%" /c /O2 /MT /nologo /W3 /std:c++20 /EHsc /d2FH4- ^
  ...
  /Fo"%OBJ_DIR%\\" ^
  "%SRC_DIR%\Main.cpp" ^
  "%SRC_DIR%\HuntConfig.cpp" ^
  "%SRC_DIR%\HuntRegistry.cpp" ^
  "%SRC_DIR%\HuntHttpClient.cpp" ^
  "%SRC_DIR%\HuntHooks.cpp" ^
  "%SRC_DIR%\HuntCommands.cpp" ^
  "%SRC_DIR%\HuntPerms.cpp" ^
  "%SRC_DIR%\HuntWorld.cpp" ^
  "%SRC_DIR%\HuntLifecycle.cpp"
if %ERRORLEVEL% neq 0 goto :error

echo === Linking DLL ===
"%LINK_EXE%" /DLL /NOLOGO ^
  /OUT:"%BIN_DIR%\ArkEventHunt.dll" ^
  /LIBPATH:"%LIB_DIR%" ^
  /LIBPATH:"%WIN_LIB%" ^
  /LIBPATH:"%WIN_SDK_LIB%\ucrt\x64" ^
  /LIBPATH:"%WIN_SDK_LIB%\um\x64" ^
  ArkApi.lib ^
  kernel32.lib user32.lib advapi32.lib ole32.lib oleaut32.lib winhttp.lib ^
  "%OBJ_DIR%\Main.obj" ^
  "%OBJ_DIR%\HuntConfig.obj" ^
  "%OBJ_DIR%\HuntRegistry.obj" ^
  "%OBJ_DIR%\HuntHttpClient.obj" ^
  "%OBJ_DIR%\HuntHooks.obj" ^
  "%OBJ_DIR%\HuntCommands.obj" ^
  "%OBJ_DIR%\HuntPerms.obj" ^
  "%OBJ_DIR%\HuntWorld.obj" ^
  "%OBJ_DIR%\HuntLifecycle.obj"
if %ERRORLEVEL% neq 0 goto :error
...
echo Output: %BIN_DIR%\ArkEventHunt.dll
goto :end
